...
typealias CCSegments = [CCSegment]
...
func cubicCurveSegments(from points:CGPoints) -> CCSegments {
    let controlPoints = controlPointsFromPoints(dataPoints: points)
    var result = CCSegments()
    for i in 1..<points.count {
        let start = points[i-1]
        let end = points[i]
        let segControls = controlPoints[i-1]
        result.append(CCSegment(
            start: start,
            end: end,
            startControl: segControls.controlPoint1,
            endControl: segControls.controlPoint2))
    }
    return result
}
...
func controlPointsFromPoints(dataPoints: [CGPoint]) -> [CubicCurveSegment] {
    ...
    if count == 1 {
        ...
        for i in 0..<count {
            let P3 = dataPoints[i+1]
            let controlPointX:CGFloat
            let controlPointY:CGFloat
            
            if i == count-1 {
                guard let P1 = firstControlPoints[i] else{
                    continue
                }
                controlPointX = (P3.x + P1.x)/2
                controlPointY = (P3.y + P1.y)/2
            } else {
                guard let nextP1 = firstControlPoints[i+1] else {
                    continue
                }
                controlPointX = 2*P3.x - nextP1.x
                controlPointY = 2*P3.y - nextP1.y
            }
